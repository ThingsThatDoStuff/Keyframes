...
interface KfPropertyRotation extends KfAnimatable<[number] | [number, number, number]> {
  property: "ROTATION",
  anchor: KfPoint
};
...
class KfDrawable extends React.Component<KfDrawableProps> {
  static defaultProps = {
    visible: true
  };
...
function sortAnimations(
  { property: aProp }: KfProperty,
  { property: bProp }: KfProperty
): number {
    ...
    return (
      <div>
        ...
        {((this.props.shouldRender || this.state.shouldRenderStuff) &&
          ...
          <div>
            <button onClick={e => this.setState({ shouldRenderStuff: true })}>
              Initialize
            </button>
          </div>
        )}
