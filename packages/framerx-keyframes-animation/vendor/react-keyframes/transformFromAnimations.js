...
export function transformFromAnimations(animations, currentFrameNumber, blend) {
    if (!(animations && animations.length > 0)) {
        return;
    }
    var transform = new Transform();
    animations.sort(sortAnimations).forEach(function (anim) {
        var anchor = anim.anchor, property = anim.property, timing_curves = anim.timing_curves, key_values = anim.key_values;
        var values = getValueForCurrentFrame(key_values, timing_curves, currentFrameNumber, blend);
        if (values == null) {
            return;
        }
        switch (anim.property) {
            ...
            case "POSITION":
                if (anchor) {
                    ...
                    var defaultAnchor = key_values[0].data;
...
//# sourceMappingURL=transformFromAnimations.js.map